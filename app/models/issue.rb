...
class Issue < ActiveRecord::Base
  ...
  # Returns additional users provided in non-standard fields
  def additional_users
    users = []
    custom_fields = editable_custom_fields
    custom_fields.each do |field|
      if field.notify?
        field.custom_values.each do |value|
          user = Principal.detect_by_keyword(self.assignable_users, value.to_s)
          if user != nil
            users << user
          end
        end
  ...
  def notified_users
    ...
    notified = [author, assigned_to, previous_assignee, additional_users].compact.uniq
